...
function FilterView({ allItems, setShowedItems }) {
  ...
  useEffect(() => {
    setField({
      type: allItems[0].fields[0].type,
      name: allItems[0].fields[0].name,
      index: 0,
    });
    ...
    filters.forEach((filter) => {
      items = items.filter((item) => {
        if (filter.type === "number") {
          ...
          return filter.filterType === "less"
            ? new Date(item.fields[filter.index].value).getTime() <
    ...
    <Row>
      <Col xs={12}>
        <Row className={"justify-content-between"}>
              ...
              >
                {field
                  ? field.type === "number" || field.type === "date"
                    ? [
                        <option value={"more"} key={0}>
                          {language[lang].moreThen}
                        </option>,
                        <option value={"less"} key={1}>
                          {language[lang].lessThen}
                        </option>,
                      ]
                    : field.type === "checkbox"
                    ? [
                        <option value={"true"} key={2}>
                          {language[lang].onlyOn}
                        </option>,
                        <option value={"false"} key={3}>
                          {language[lang].onlyOff}
                        </option>,
                      ]
                    : []
                  : []}
          ...
          <Col
            className={
              "d-flex align-items-end px-3 justify-content-center mt-2 mt-md-0"
            }
          >
            <Button onClick={addFilter} variant={"outline-primary"}>
              {language[lang].addFilter}
            </Button>
          </Col>
        </Row>
      </Col>
              ...
              filters.map((e, i) => {
                return (
                  ...
                  >
                    <span>
                      {e.type !== "checkbox"
                        ? `${e.name} filter ${e.filterType} then ${
                            e.value && e.value
                          }`
