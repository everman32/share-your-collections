...
function Info({ info, loadCollection, fields, id, edit }) {
  ...
  async function createItem(item, file) {
    const loadFile = await request("/api/files/fileUpload", "POST", file);
        ...
        >
          <Row className={"justify-content-center"}>
            <Col lg={7} md={7} xs={12}>
              ...
              <p>
                <strong>{languages[lang].collections.owner}</strong>:{" "}
                <a href={"/collections/" + info.ownerId}>{info.ownerName}</a>
              </p>
              <p>
                <strong>{languages[lang].theme}</strong>:{" "}
                {languages[lang][info.theme.toLowerCase()]}
              </p>
            </Col>
            <Col
              style={{
