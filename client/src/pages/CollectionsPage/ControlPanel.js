...
function ControlPanel ({ owner, loadCollections }) {
  ...
  return (
    <Navbar
      className={
        'px-mg-5 px-4 mt-2 justify-content-between ' +
        'bg-' +
    ...
    >
      <Row>
        <span>
          <strong>{language[lang].collections.owner}</strong>: {owner.username}
        </span>
      </Row>
      <Row>
        {role === 'Admin' || userId === owner._id
          ? (
            <Button variant='outline-success' onClick={() => setShowModal(true)}>
              {language[lang].collections.create}{' '}
              <span className='d-none d-sm-inline'>
                {language[lang].collections.collection2}
              </span>
            </Button>
            )
          : (
