...
router.post('/login', async (req, res) => {
  try {
    if (req.body.type === 'common') {
      ...
      const token = jwt.sign({ userId: user._id }, process.env.SECKET_KEY, {
        expiresIn: '6h'
      })
      ...
      if (user) {
        ...
        return res
          .status(200)
          .json({ token, message: 'Пользователь уже записан', user, ok: true })
      }
      ...
      const token = jwt.sign({ userId: newUser._id }, process.env.SECKET_KEY, {
        expiresIn: '6h'
      })
...
router.post('/auth', [isAuth, isBlocked], (req, res) => {
  res
    .status(200)
    .json({ message: 'Вы авторизованы', ok: true, user: req.user })
})
