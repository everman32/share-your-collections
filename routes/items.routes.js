...
router.post('/editItem', isAuth, async (req, res) => {
  try {
    if (
      req.user.role !== 'Admin' &&
    ...
    res
      .status(200)
      .json({ message: 'Редактирование прошло успешно', item, ok: true })
  } catch (e) {
    res.status(500).json({ message: 'Что-то пошло не так', error: e.message })
  }
})
...
router.post('/editItemFields', isAuth, async (req, res) => {
  try {
    if (
      req.user.role !== 'Admin' &&
      req.user._id.toString() !== req.body.ownerId
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let j = 0; j < items[i].tags.length; j++) {
        const tag = await tagModel.findOne({ _id: items[i].tags[j] })
        if (tag) itemTags.push(tag)
      }
      for (const k in tags) {
        for (const j in itemTags) {
    ...
    const searcher = new textSearch(items, [
      'name',
      'parentName',
      'fields.value',
      'fields.name',
...
router.post('/deleteItem', isAuth, async (req, res) => {
  try {
    if (
      req.user.role !== 'Admin' &&
      req.user._id.toString() !== req.body.ownerId
    ...
    for (let i = 0; i < item.tags.length; i++) {
      const tag = await tagModel.findOne({ _id: item.tags[i] })
      tag.value -= 1
      if (tag.value <= 0) {
...
router.post('/likeItem', isAuth, async (req, res) => {
  try {
    const item = await itemModel.findOne({ _id: req.body.id })
